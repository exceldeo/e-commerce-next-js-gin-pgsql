...
const AddressList = ({ selected, setSelected, setEdit }) => {
  ...
  const setDefaultAddressHandler = (id) => {
    if (selected?.id === id || defaultAddress?.id === id) {
      setSelected ? setSelected(null) : setDefaultAddress(null);
      editAddress.mutate({ id, is_default: false });
      return;
    }

    const address = data?.find((address) => address?.id === id);
    setSelected ? setSelected(address) : setDefaultAddress(address);
    editAddress.mutate({ id, is_default: true });
  };
  ...
  if (deleteAddress.isError) {
    toast.error(
      deleteAddress?.error?.response?.data?.message ??
        deleteAddress.error.message
    );
  }
          ...
          >
            <div className='flex items-center justify-between'>
              ...
              <div className='flex items-center space-x-2.5 rtl:space-x-reverse'>
                <button
                  onClick={() => setEdit(address.id)}
                  type='button'
                  className='flex h-[34px] w-[34px] items-center justify-center rounded-full border border-qgray text-qyellow'
                >
                  <svg
                ...
                <button
                  ...
                  type='button'
                  className='flex h-[34px] w-[34px] items-center justify-center rounded-full border border-qgray'
                >
                  <svg
                    width='17'
                    height='19'
